...
void leituraDados()
{
  ...
  char codigo[MAX_SIZE_BLOCK] = "1442473        ";
  char bufferC[MAX_SIZE_BLOCK] = "              ";
  char result[MAX_SIZE_BLOCK];
  int contigual=0;
  ...
  for (uint8_t i = 0; i < MAX_SIZE_BLOCK; i++)
  {
      ...
      if(bufferC[i]==codigo[i]){
        Serial.println(" Igual em : "+String(i));
        contigual++;
      }
      else{
        if(codigo[i]==' '){
          Serial.println("Diferente em : "+String(i)+" : "+String(codigo[i])+" : "+String(bufferC[i]));
        }
      }
  }
  if(contigual>=15){
  ...
  if(strcmp(bufferC, codigo)==32){
    Serial.println("OK");
    hmi.setPage(2);
    String nome = "Peterson";
    String codigoString = codigo;
    hmi.setText(0x0200, nome);
    hmi.setText(0x0210, codigoString);
  }
  Serial.println("Resultado : "+String(strcmp(bufferC, codigo)));
}
